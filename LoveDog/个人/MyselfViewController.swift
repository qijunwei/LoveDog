...
class MyselfViewController: UIViewController,MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //推送设置暂时不加
        dataArr = [["我的收藏","扫一扫"],["评价","声明"],["清理缓存","意见反馈","联系我们"]]
    ...
    func createTableView(){
        ...
        headView = UIView.init(frame: CGRect(x: 0, y: 64, width: SCREEN_W, height: 120))
        let logoview = UIImageView.init(frame: CGRect(x: (SCREEN_W - 100) / 2, y: 0, width: 100, height: 100))
        logoview.image = UIImage.init(named: "graylogo")
        headView.addSubview(logoview)
        let logolabel = UILabel.init(frame: CGRect(x: (SCREEN_W - 80) / 2, y: 100, width: 100, height: 10))
