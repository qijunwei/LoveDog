...
open class LBXScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [AnyObject]!, from connection: AVCaptureConnection!)
    {
        if !isNeedScanResult
        {
            //上一帧处理中
    ...
    open func captureImage()
    {
        ...
        stillImageOutput?.captureStillImageAsynchronously(from: stillImageConnection, completionHandler: { (imageDataSampleBuffer, error) -> Void in
            ...
            {
                let imageData:Foundation.Data = AVCaptureStillImageOutput.jpegStillImageNSDataRepresentation(imageDataSampleBuffer)
